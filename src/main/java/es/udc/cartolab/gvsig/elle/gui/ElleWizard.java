...
public class ElleWizard extends WizardPanel {
	...
	public void initWizard() {
		...
		if (dbs != null) {
			mainPanel.setBorder(javax.swing.BorderFactory.createTitledBorder(null, _("Choose_Layer"),
	...
	private JPanel getListPanel() {
		if (listPanel == null) {
			...
			try {
				InputStream stream = getClass().getClassLoader().getResourceAsStream("forms/loadLayer.jfrm");
							...
							if (selected.length == 1) {
								...
								try {
									layers = dbs.getTable(DBStructure.getMapTable(), DBStructure.getSchema(), where,
											new String[] { "posicion", "nombre_capa" }, true);
