...
public class SaveLegendsWizardComponent extends WizardComponent {
	...
	private static final Logger logger = LoggerFactory.getLogger(SaveLegendsWizardComponent.class);
	...
	private JPanel getOptionsPanel() {
		...
		panelOptions.setLayout(new MigLayout("", "[grow]80", "[][]15[][]15[][]"));
	...
	private JPanel getOverviewPanel() {
		...
		overviewPanel.setLayout(new MigLayout("", "[grow, left][][shrink, right]", "[]"));
	...
	private JPanel getDBPanel() {
		...
		MigLayout layout = new MigLayout("inset 0, align center", "10[][grow, right]10", "5[grow]5");
		...
		if (DBSession.getCurrentSession() != null) {
			...
			Object mapName = properties.get(SaveMapWizardComponent.PROPERTY_MAP_NAME);
			dbStyles = new JTextField(mapName == null ? "" : mapName.toString(), 20);
	...
	private boolean useNotGVL() {
		...
		int n = JOptionPane.showOptionDialog(this, _("legend_format_question"), null, JOptionPane.YES_NO_CANCEL_OPTION,
	...
	private boolean overwriteLegends(AbstractLegendsManager legendsManager) {
		if (legendsManager.exists()) {
			...
			int n = JOptionPane.showOptionDialog(this, message, _("overwrite_legend"), JOptionPane.YES_NO_CANCEL_OPTION,
					JOptionPane.WARNING_MESSAGE, null, options, options[1]);
			if (n != 0) {
				return false;
			} else {
	...
	public void finish() throws WizardException {
		...
		if (overviewCHB.isSelected()) {
			...
			if (aux != null && aux instanceof IView) {
				FLayers ovLayers = ((IView) aux).getMapOverview().getMapContext().getLayers();
	...
	public void showComponent() throws WizardException {
		...
		for (LayerProperties lp : layers) {
			Object[] row = { lp.save(), lp.getLayername(), type };
			model.addRow(row);
		}
